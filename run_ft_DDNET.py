...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,get_cosine_schedule_with_warmup, GPT2Config)
...
from models.model import GPT2VisionAttentiveLMHead1 #for original model
from models.model_one_modal_evt import GPT2VisionAttentiveLMHeadEvt # for one modality -evt
from models.model_one_modal_plc import GPT2VisionAttentiveLMHeadPlc # for one modality -plc
from models.model_one_modal_img import GPT2VisionAttentiveLMHeadImg # for one modality -img
...
from utils.parallel2 import DataParallelModel2, DataParallelCriterion2
from utils.parallel3 import DataParallelCriterion3 # for evaluation 
...
MODEL_CLASSES = {
    'gpt2_vc': (GPT2Config, GPT2VisionAttentiveLMHead1, VisualCometTokenizer),
}
...
def train(args, dataset, model, tokenizer):
    ...
    ######## one modal ########
    model_evt = GPT2VisionAttentiveLMHeadEvt.from_pretrained('exp_evt')
    model_plc = GPT2VisionAttentiveLMHeadPlc.from_pretrained('exp_plc')
    model_img = GPT2VisionAttentiveLMHeadImg.from_pretrained('exp_img')

    model_evt.to(args.device)
    model_plc.to(args.device)
    model_img.to(args.device)
    ...
    if args.n_gpu > 1:
        ...
        model_evt = DataParallelModel2(model_evt)
        model_plc = DataParallelModel2(model_plc)
        model_img = DataParallelModel2(model_img)
    ...
    criterion = torch.nn.CrossEntropyLoss(ignore_index=-1)  #add
    criterion = DataParallelCriterion(criterion)  #add

    criterion2 = torch.nn.CrossEntropyLoss(ignore_index=-1, reduction = 'none')
    criterion2 = DataParallelCriterion2(criterion2)
    ...
    for epoch in train_iterator:
        ...
        for step, data_input in enumerate(epoch_iterator):
            ...
            plc_inference_start_token_idx = tokenizer.convert_tokens_to_ids([tokenizer.begin_place])[0]
            plc_inference_end_token_idx = tokenizer.convert_tokens_to_ids([tokenizer.end_place])[0]

            evt_inference_start_token_idx = tokenizer.convert_tokens_to_ids([tokenizer.begin_event])[0]
            evt_inference_end_token_idx = tokenizer.convert_tokens_to_ids([tokenizer.end_event])[0]
            ...
            for a in inputs :
                ...
                start_idx = a.index(plc_inference_start_token_idx) #evt only
                end_idx = a.index(plc_inference_end_token_idx)
                a[start_idx: end_idx + 1] = [unk_id] * (end_idx - start_idx + 1)
                ex_evt.append(a)
                
                start_idx = b.index(evt_inference_start_token_idx) #plc only
                end_idx = b.index(evt_inference_end_token_idx)
                b[start_idx: end_idx + 1] = [unk_id] * (end_idx - start_idx + 1)
                ex_plc.append(b)
                
                start_idx = c.index(evt_inference_start_token_idx) #img only
                end_idx = c.index(plc_inference_end_token_idx)
                c[start_idx: end_idx + 1] = [unk_id] * (end_idx - start_idx + 1)
                ex_img.append(c)
                
            ex_evt = torch.tensor(ex_evt)
            ex_evt.to(args.device)
                        
            ex_plc = torch.tensor(ex_plc)
            ex_plc.to(args.device)

            ex_img = torch.tensor(ex_img)
            ex_img.to(args.device)
            
            #ipdb.set_trace()# 1

                ########### one modal ##########
            with torch.no_grad() :
                output_evt = model_evt(ex_evt, labels=labels, img_feats=None, boxes=None, boxes_mask=None, objects=None, segments=None,
                                    person_ids=None, subject_ids=None) #output : (n_gpu, batch x 127, 50137) / ex_evt : (n_gpu x batch, 128)
                    
                output_plc = model_plc(ex_plc, labels=labels, img_feats=None, boxes=None, boxes_mask=None, objects=None, segments=None,
                                    person_ids=None, subject_ids=None)

                output_img = model_img(ex_img, labels=labels, img_feats=img_feats, boxes=boxes, boxes_mask=boxes_mask, objects=objects, segments=segments,
                                person_ids=person_ids, subject_ids=subject_ids)

                shift_labels = labels[..., 1:].contiguous() #add 
                shift_labels = shift_labels.view(-1) #add  (n_gpu x batch x 127)

                
                loss_evt = criterion2(output_evt, shift_labels) #loss_evt = [tensor(), tensor()]  # Q1.with torch.no_grad() apply ? 
                loss_plc = criterion2(output_plc, shift_labels)
                loss_img = criterion2(output_img, shift_labels) 
                
            loss1_e = []
            loss2_p = []
            loss3_i = []
            
            #ipdb.set_trace() #2 / 1~2 : 44secs
                
            for l in loss_evt : 
                tmp = torch.chunk(l, args.per_gpu_train_batch_size, dim=0)
                for ll in tmp : 
                    avg = torch.mean(ll[ll != 0])
                    loss1_e.append(avg.item()) # avg : n_gpu x batch_size
                    
            for l in loss_plc : 
                tmp = torch.chunk(l, args.per_gpu_train_batch_size, dim=0)
                for ll in tmp : 
                    avg = torch.mean(ll[ll != 0])
                    loss2_p.append(avg.item()) 
            
            for l in loss_img : 
                tmp = torch.chunk(l, args.per_gpu_train_batch_size, dim=0)
                for ll in tmp : 
                    avg = torch.mean(ll[ll != 0])
                    loss3_i.append(avg.item()) 
        
            
            loss1_e = torch.tensor(loss1_e)   # size : (n_gpu x batch)
            loss2_p = torch.tensor(loss2_p)
            loss3_i = torch.tensor(loss3_i)
            
            loss1_e = loss1_e.view(args.train_batch_size,1).to(args.device) # (n_gpu x batch , 1)
            loss2_p = loss2_p.view(args.train_batch_size,1).to(args.device)
            loss3_i = loss3_i.view(args.train_batch_size,1).to(args.device)

            ################################
            
            #ipdb.set_trace() # 3 / 2-3 : 1.5 secs
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1,
                                       0] and args.logging_steps > 0 and global_step % args.logging_steps == 0: #logging_step 500
                ...
                if args.local_rank in [-1,
                                       0] and args.save_steps > 0 and global_step % args.save_steps == 0: # per iteration 10000 / need to retouch
                    ...
                    #results = evaluate(args, model, model_evt, model_plc, model_img, tokenizer, dataset, criterion2, postfix='{}_{}'.format(epoch, global_step))
...
def evaluate(args, model, model_evt, model_plc, model_img, tokenizer,
             dataset, criterion2, 
             postfix=""):
    ...
    criterion = torch.nn.CrossEntropyLoss(ignore_index=-1)  #add
    criterion = DataParallelCriterion3(criterion)  #add
    ...
    for data_input in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for l in loss_evt : 
            tmp = torch.chunk(l, args.per_gpu_eval_batch_size, dim=0)
            for ll in tmp : 
                avg = torch.mean(ll[ll != 0])
                loss1_e.append(avg.item()) # avg : n_gpu x batch_size
        ...
        loss1_e = loss1_e.view(args.eval_batch_size,1).to(args.device) # (n_gpu x batch , 1)
        loss2_p = loss2_p.view(args.eval_batch_size,1).to(args.device)
        loss3_i = loss3_i.view(args.eval_batch_size,1).to(args.device)
        ...
        with torch.no_grad():
            outputs = model(inputs, labels=labels, img_feats=img_feats, boxes=boxes, boxes_mask=boxes_mask, objects=objects, segments=segments,
                                    person_ids=person_ids, subject_ids=subject_ids, loss1_e=loss1_e, loss2_p=loss2_p, loss3_i=loss3_i)

            shift_labels = labels[..., 1:].contiguous()

            #ipdb.set_trace()
            lm_loss = criterion(outputs, shift_labels.view(-1))
            #lm_loss = outputs[0]
...
def main():
    ...
    model = model_class.from_pretrained(args.model_name_or_path, #gpt2
                                        from_tf=bool('.ckpt' in args.model_name_or_path),
                                        config=config)
    model.resize_token_embeddings(len(tokenizer)) #len(tokenizer ?) 50317
