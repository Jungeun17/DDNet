...
class Causal_intervention(nn.Module):
    ...
    def forward(self, x, loss1_e, loss2_p, loss3_i): 
        ...
        #loss
        comb_loss = torch.cat((loss1_e, loss2_p, loss3_i), dim=1)
        comb_loss = torch.argmin(comb_loss,dim=1).tolist()
        
        final_causal_att = torch.zeros_like(att_causal_evt)
        
        #ipdb.set_trace()

        for i, num in enumerate(comb_loss):
            if num == 0 :
        ...
        '''
        #### delete     
        comb_loss = nn.Softmax(dim=1)(-comb_loss) #inverse
        
        tmp = []
        final_causal_att = torch.zeros_like(att_causal_evt)
        for i in range(len(att_causal_evt)): #batch
            for j in range(len(att_causal_evt[0])) : #128
                tmp1 = comb_loss[i][0] * att_causal_evt[i][j] #len = 768 #
                tmp2 = comb_loss[i][1] * att_causal_plc[i][j]
                tmp3 = comb_loss[i][2] * att_causal_img[i][j]
...
class GPT2VisionAttentiveTransformer(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids,
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past)):
            ...
            #causal
            hidden_states = hidden_states + self.causal_att(hidden_states, loss1_e, loss2_p, loss3_i)  
...
class GPT2VisionAttentiveLMHead1(GPT2PreTrainedModel):
    def __init__(self, config):
        super(GPT2VisionAttentiveLMHead1, self).__init__(config)
    ...
    def forward(self,
        ...
        ### Edited by JYKIM ### 
        lm_logit = self.lm_head(transformer_outputs[0])# + self.final_logits_bias   
        #print(lm_logit.size()) #[64,128,50317]
        shift_logits = lm_logit[..., :-1, :].contiguous()
        #print(shift_logits.size()) #[64,127,50317]
        shift_logits = shift_logits.view(-1, shift_logits.size(-1))
        #print(shift_logits.size()) #[8128,50317]
		### 
    ...
    """                                           
        ...
        if labels is not None:
            ...
            if use_rank:
                ...
                #loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)),
                                #shift_labels.view(-1))
                logits_out = shift_logits.view(-1, shift_logits.size(-1))
    ...
    #         self.z1_img = nn.Parameter(torch.from_numpy(conf_np).cuda().to(torch.float32).requires_grad_()) 

    #         #evt
    #         with open('/workspace/je/vc-causal/models/conf_100_fdata_noun.pkl','rb') as f :
    #             conf = pickle.load(f)
    #         conf_np = np.array(conf)
    #         self.z1_evt = nn.Parameter(torch.from_numpy(conf_np).cuda().to(torch.float32).requires_grad_()) 

    #         #plc
    #         with open('/workspace/je/vc-causal/models/place_150.pkl','rb') as f :
    #             conf = pickle.load(f)
    #         conf_np = np.array(conf)
    #         self.z1_plc = nn.Parameter(torch.from_numpy(conf_np).cuda().to(torch.float32).requires_grad_()) 

    #         ##
    #         self.z1_k = nn.Linear(768, config.n_embd) # need to edit
    #         self.z1_v = nn.Linear(768, config.n_embd)

    #         self.evt_k = nn.Linear(300, config.n_embd)
    #         self.evt_v = nn.Linear(300, config.n_embd)

    #         self.s_q = nn.Linear(config.n_embd, config.n_embd)

    #     def forward(self, x, loss_bias): #loss_bias = [img, evt, plc]

    #         loss_bias = loss_bias.tolist()
    #         min_index = loss_bias[0].index(min(loss_bias[0]))
            
    #         if min_index == 0 : #img
    #             z1 = self.z1_k(self.z1_img)
    ...
    #         if min_index ==2 : #plc
    #             z1 = self.z1_k(self.z1_plc)
    #             att_S = torch.matmul(x, torch.t(z1))/ math.sqrt(x.size(-1))
    #             att_S = nn.Softmax(dim=-1)(att_S)
    #             S = torch.matmul(att_S, self.z1_v(self.z1_plc))
    ...
    #         #ipdb.set_trace()
    #         hidden_states = hidden_states + self.causal_att(hidden_states, loss_bias)  
    #         hidden_states = self.ln_f(hidden_states)
